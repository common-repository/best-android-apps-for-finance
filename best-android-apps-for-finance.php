...
//  Adding Action
add_action('plugins_loaded','finance_display_widget');

//  Display Widget
function finance_display_widget() {
    register_sidebar_widget(__('Best Android apps for Finance Widget'),'finance_rawapps_widget');
    register_widget_control('Best Android apps for Finance Widget','finance_widget_options');

}
//  Creating RawApps  widget
function finance_rawapps_widget($args) {
    extract($args);
    $options=get_option("finance_rawapps_widget");
    if (!is_array($options)) {
	    $options=array(
    ...
    finance_rawapps_widget_template();
    echo $after_widget;
   ...
   //   Widget template
    function finance_rawapps_widget_template() {include_once(WP_PLUGIN_DIR."/best-android-apps-for-finance/rawapps_widget_template.php");}

   //   Widget Control Panel
    function finance_widget_options()
    {
	...
	if ($_POST['finance-rawapps-widget-submit']) {
	    ...
	    update_option("finance_rawapps_widget",$options);
	}
	//Form
	...
	echo "<input type='hidden' id='finance-rawapps-widget-submit' name='finance-rawapps-widget-submit' value='1' />";
